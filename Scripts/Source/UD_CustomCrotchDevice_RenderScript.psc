...
Function InitPost()
    parent.InitPost()
    UD_DeviceType = "(Crotch)Arousing Belt"
    _AMKey = ("ArMov." + GetDeviceName())
    OrgasmSystem.AddOrgasmChange(GetWearer(),_AMKey,0x40,0x3F)
    OrgasmSystem.UpdateOrgasmChangeVar(GetWearer(),_AMKey,1,4.0,1) ;increase orgasm rate to 5 when moving
    OrgasmSystem.UpdateOrgasmChangeVar(GetWearer(),_AMKey,9,4.0,1) ;increase arousal rate to 5 when moving
    
EndFunction

Function onRemoveDevicePost(Actor akActor)
    OrgasmSystem.RemoveOrgasmChange(akActor,_AMKey)
