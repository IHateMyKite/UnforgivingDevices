;/  File: UD_ModTrigger_SCastValue
    ...
    Parameters in DataStr:
        [0]     Int     The cost of all spells that actor must cast for the trigger to work
        
        [1]     Int     (optional) Repeat
                        Default value: 0 (False)
                        
        [2]     Int     (script) The cost of all spells that actor cast so far
                        If it is negative then trigger will never occur

    Example:

/;
ScriptName UD_ModTrigger_SCastValue extends UD_ModTrigger
...
Bool Function SpellCast(UD_Modifier_Combo akModifier, UD_CustomDevice_RenderScript akDevice, Spell akSpell, String aiDataStr)
    ...
    If loc_cost_current >= loc_cost_needed
        ...
        Return True
    Else 
        akDevice.editStringModifier(akModifier.NameAlias, 2, loc_cost_current)
        Return False
    EndIf
EndFunction
