...
Function onRemoveDevicePost(Actor akActor)
    If getRelativeCharge() >= 1.0 ; Fully charged.
        ...
        if UD_ChargeRewardFull
            if WearerIsPlayer()
                UDmain.Print("The removed plug breaks into " + UD_ChargeRewardFull.getName(),1) ;need to really shorten this one, otherwise it's too long and unreadable. maybe break it into 2 strings?
            endif
    ...
    Else
        if UD_ChargeRewardEmpty
            if WearerIsPlayer()
                UDmain.Print("Removed plug darkens and breaks into " + UD_ChargeRewardEmpty.getName()) ;same shortening here
            endif
